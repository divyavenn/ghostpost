...
async def discover_recently_posted(username: str, user_handle: str, max_tweets: int = 50) -> dict[str, Any]:
    ...
    """
    from backend.data.twitter.comments_cache import process_scraped_replies
    ...
    try:
        ...
        for tweet in recent_tweets:
            ...
            try:
                ...
                # Deep scrape if active state to get initial replies
        ...
        notify(f"✅ [discover_recently_posted] Complete: {results['discovered_tweets']} tweets, {results['new_comments']} comments")
...
async def discover_engagement(username: str, user_handle: str) -> dict[str, Any]:
    ...
    try:
        ...
        for tweet in active_tweets:
            ...
            try:
                ...
                if tweet_id in tweets_map:
                    ...
                    # Update activity if new comments found
                    if new_comment_ids:
        ...
        notify(f"✅ [discover_engagement] Complete: {results['active_scraped']} deep, {results['warm_scraped']} shallow, {results['new_comments']} comments")
...
async def discover_resurrected(username: str, user_handle: str) -> dict[str, Any]:
    ...
    try:
        ...
        user_tweet_ids = get_user_tweet_ids(username)
        ...
        notify(f"✅ [discover_resurrected] Complete: {results['resurrected_tweets']} resurrected, {results['new_comments']} comments")
