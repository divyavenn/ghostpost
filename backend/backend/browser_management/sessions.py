...
Allows users to login via browser on backend with Chrome extension monitoring state.
"""
import secrets
import time
...
async def create_browser_session(username: str) -> dict:
    ...
    # Launch browser with remote debugging enabled and Chrome extension loaded
    # Extension monitors browser state and communicates with backend
    browser = await playwright.chromium.launch(
        headless=False,  # User needs to see browser (Chrome extension monitors state)
        args=[
...
async def save_and_close_session(session_id: str) -> dict:
    ...
    """
    from backend.twitter.authentication import exchange_code_for_token
    from backend.utlils.utils import error, store_browser_state
    session = active_sessions.get(session_id)
    if not session:
    ...
    if "code=" in current_url:
        ...
        if code:
            ...
            # Store tokens
            from backend.utlils.utils import store_token
