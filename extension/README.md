...
A Chrome extension that converts web content into clean Markdown files with a single click.


## The Stack

The backend runs on FastAPI with Modal deployment, which gives me serverless compute that scales automatically. Modal's great for this because conversion jobs are bursty - sometimes you're processing dozens of articles, sometimes nothing for hours.

The frontend is React with a clean, minimal interface. One button, one action. The extension communicates with the backend via WebSockets for real-time conversion status updates.

For audio transcription (YouTube videos), I'm using OpenAI's Whisper model. It's surprisingly accurate and handles multiple languages without additional configuration.
...
Different content types need completely different processing pipelines:

**Substack articles**: HTML parsing with BeautifulSoup, extracting the article body while stripping ads and sidebars. Formatting preservation was key - code blocks, quotes, and inline images all needed to survive the conversion.

**X threads**: Twitter's structure is nested and non-linear. I had to reconstruct thread order from the DOM, handle quote tweets, and preserve media links without bloating the output file.


**YouTube videos**: Download audio stream, run through Whisper for transcription, add timestamps. The scipy library handles audio processing and resampling to formats Whisper expects.
...
Modal handles cold starts really well, which matters for a tool people use sporadically. Functions spin up in under a second, and I can specify exact resource requirements per job type. YouTube transcription gets a GPU, article conversions run on CPUs.

The cost model is pay-per-use, so I'm not burning money on idle servers. Perfect for a side project that might see 10 conversions or 1000 in a day.


## Markdown Normalization
Getting consistent, clean Markdown output was harder than expected. Every source has different HTML quirks, edge cases, and formatting conventions.
