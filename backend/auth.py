...
class SupabaseAuthManager:
        ...
        try:
            ...
            if response.user:
                return {
                    ...
                    "username": response.user.user_metadata.get("username", response.user.email),
    ...
    def create_user_profile(self, user_id: str, email: str, username: str = None, role: str = "user") -> bool:
    ...
    def get_user_stats(self) -> Dict[str, int]:
        ...
        try:
            ...
            active_users_response = self.supabase.table("profiles").select("id", count="exact").eq("is_active", True).execute()
            active_users = active_users_response.count or 0
            
            # Get admin users
            admin_users_response = self.supabase.table("profiles").select("id", count="exact").eq("role", "admin").execute()
...
def require_admin(current_user: Dict[str, Any] = Depends(verify_token)) -> Dict[str, Any]:
    """Require admin role"""
